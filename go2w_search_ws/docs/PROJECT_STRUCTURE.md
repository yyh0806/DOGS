...
| 2 建图 | slam_toolbox 2D / FAST_LIO 3D | ⏳ **被 odom 缺口阻塞**（见 P0）|
| 3 Nav2 自主导航 | nav2_slim | ⏳ 待建图完成 |
| 4 AI 迁移 | nx_ai_node（YOLO/VLM） | ✅ 视频通，检测待接 locateanything |

## 五、关键待解决（P0 — 阻塞建图）

1. **`nx_sensor_node.py` 的 `/odom` xy 占位**（代码注释"留 FAST_LIO"，恒 0）→ slam_toolbox 无平移 odom 不建图。
   - 解法 A：改 nx_sensor 补轮速积分（motor_state[12-15] 已确认是 4 轮，~10 行）
   - 解法 B：点亮独立 Livox → FAST_LIO 产 odom（精度更高）
2. **Livox MID360 雷达未点亮**（用 NX USB 口供电 5V/0.9A < 需 5V/3A；数据应走 RJ45 非 USB）。
   - 解法：独立 PD 电源供电 + 网线连 NX `enP8p1s0` + 配 host IP 192.168.1.5
