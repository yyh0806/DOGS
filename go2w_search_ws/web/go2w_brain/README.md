...
| `platform.py` | 世界状态只读接入: NxHttpAdapter / MockAdapter |
| `brain_loop.py` | 事件驱动自由循环 + 规则退化 |
| `tools/` | M1 只读工具: get_gps / get_battery / get_pose / speak(桩) |
...
# 真机/仿真 (NX 上, 读 nx_web_server 状态):
python -m go2w_brain.run_brain --task "报告当前状态" --platform nx \
    --nx-url http://localhost:8000 --no-llm

# 有 DEEPSEEK_API_KEY 时省略 --no-llm 即走 LLM function-calling
```
